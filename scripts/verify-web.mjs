...
checks.push(['Login keeps ledger fetch off the critical path',!index.includes('preloadDashboardData(user.uid)')&&!index.includes('bsp_dashboard_prefetched')&&index.includes('bsp_user_prefetched')&&!index.includes('loginDataLoading')]);
...
checks.push(['Dashboard hydrates through Firestore listeners',dashboard.includes('function setupListeners()')&&dashboard.includes('onSnapshot({includeMetadataChanges:true}')&&dashboard.includes('initialCustomersReady=true')&&dashboard.includes('initialTransactionsReady=true')&&!dashboard.includes('async function primeDashboardData()')]);
